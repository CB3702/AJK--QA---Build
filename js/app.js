...
function scopeIcon(key){
  const icons = { screed:'🪵', cb:'🔩', raf:'🏗️', hw:'🌲', tiling:'🔲' };
  return icons[key] || '📋';
}
...
function renderFormsView(){
  ...
  // Build latest-revision map for this area + scope
  ...
  const submittedKey = {}; // "F01" → latestForm
  ...
  cfg.forms.forEach(f => {
    ...
    if(existing){
      ...
      statusChip = `<span class="status-chip status-${ls.toLowerCase()}">${ls}</span>`;
      if(ins) statusChip += ` <span class="status-chip status-${ins}">${ins}</span>`;
      if(ls === 'Submitted') cardClass += ' card-submitted';
    }
    html += `<div class="${cardClass}${isHold?' is-hold':''}" onclick="openFormOrRegister('${f.id}','${currentScope}')">
      <div class="card-ref-row">
        <span class="card-ref">${f.id}</span>
        ${isHold ? `<span class="hold-badge">🔒 Hold</span>` : ''}
      </div>
      <div class="card-title">${f.title}</div>
      <div class="card-meta">${f.freq || ''}</div>
      ${statusChip ? `<div class="card-status-row">${statusChip}</div>` : ''}
    </div>`;
  });
...
function openFormOrRegister(formCode, scopeKey){
  ...
  if(!formDef){ alert('Form definition not found.'); return; }

  const existing = getFormsForArea(currentArea.id)
...
function renderRegister(){
  ...
  let html = `
    <div class="register-form-header">
      ...
      ${isHold ? `<div class="hold-warning" style="margin-top:10px">⚠️ HOLD POINT — Formal sign-off required before works proceed.</div>` : ''}
    </div>`;

  if(latestPerRoot.length){
    html += `<div class="register-section-label">Active / Latest</div>`;
    latestPerRoot.forEach(r => { html += renderRegisterCard(r, true); });
  }

  if(olderRecords.length){
    html += `<div class="register-section-label register-section-history">Previous Revisions</div>`;
    olderRecords.forEach(r => { html += renderRegisterCard(r, false); });
  }

  html += `<div class="register-new-entry">
    <button class="btn btn-primary" style="max-width:260px" onclick="openFreshEntry()">+ New Form Entry</button>
  </div>`;
...
function renderRegisterCard(r, isLatest){
  ...
  const dateStr = r.submittedAt ? r.submittedAt.slice(0,10) : (r.savedAt ? r.savedAt.slice(0,10) : '—');
  const dateLabel = ls === 'Draft' ? 'Saved' : 'Submitted';

  let actions = '';
  if(ls === 'Draft'){
    actions = `<button class="reg-btn reg-btn-primary" onclick="continueDraft('${r.id}')">Continue Draft</button>
               <button class="reg-btn reg-btn-danger" onclick="discardDraft('${r.id}')">Discard</button>`;
  } else if(ls === 'Submitted'){
    actions = `<button class="reg-btn reg-btn-ghost" onclick="viewRecord('${r.id}')">View</button>
               <button class="reg-btn reg-btn-primary" onclick="createRevision('${rootId}')">Create Revision</button>
               <button class="reg-btn reg-btn-ghost" onclick="closeFormRecord('${r.id}')">Close</button>`;
  } else if(ls === 'Closed'){
    actions = `<button class="reg-btn reg-btn-ghost" onclick="viewRecord('${r.id}')">View</button>`;
  }

  return `<div class="register-card${isLatest ? ' register-card-latest' : ''}">
    <div class="register-card-top">
      <span class="register-card-ref">${r.formRef || '—'}</span>
      <div class="register-card-chips">
        <span class="status-chip status-${ls.toLowerCase()}">${ls}</span>
        ${ins ? `<span class="status-chip status-${ins}">${ins}</span>` : ''}
        <span class="register-card-rev">REV${String(r.revision||0).padStart(2,'0')}</span>
      </div>
    </div>
    <div class="register-card-meta">
      <span>${dateLabel}: <strong>${dateStr}</strong></span>
      <span>By: <strong>${r.submittedByName || '—'}</strong></span>
      ${r.resubmissionReason ? `<span class="register-card-reason">"${r.resubmissionReason}"</span>` : ''}
    </div>
    <div class="register-card-actions">${actions}</div>
  </div>`;
}
...
function renderFormEntry(prefill, readOnly, lifecycle){
  ...
  // After DOM is built, populate table data from prefill
  if(prefill){
    (formDef.sections || []).forEach(section => {
  ...
  if(actionsEl){
    if(readOnly){
      actionsEl.innerHTML = `<button class="btn btn-back" style="width:auto" onclick="window.history.back ? renderRegister() || showView('register') : null">← Back to Register</button>`;
    } else {
      actionsEl.innerHTML = `
...
function renderField(field, prefill, readOnly){
  ...
  switch(field.type){
    ...
    case 'select': {
      ...
      control = `<select id="f_${field.id}" ${dis}><option value="">— Select —</option>${opts}</select>`;
      break;
    }
    case 'textarea':
      control = `<textarea id="f_${field.id}" rows="3" ${dis}>${val}</textarea>`;
      break;
    case 'date':
      control = `<input type="date" id="f_${field.id}" value="${val}" ${dis}>`;
      break;
    case 'number':
      control = `<input type="number" id="f_${field.id}" value="${val}" step="any" ${dis}>`;
      break;
    default: // text
      control = `<input type="text" id="f_${field.id}" value="${val}" ${dis}>`;
  }
...
function goBack(){
  ...
  // Deepest first: register → scope (forms view) → area (scope view) → level → block → project
  const activeView = document.querySelector('.view.active');
  const activeId = activeView ? activeView.id : '';
  if(activeId === 'view-register'){
